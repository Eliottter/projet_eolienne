...
def main():
    ...
    while True:
        ...
        time.sleep(600)  # Attendre 60 secondes avant de récupérer de nouvelles données


def save_to_json():
    conn = sqlite3.connect('BDD_meteo.db')
    c = conn.cursor()
    c.execute("SELECT * FROM meteo ORDER BY DateHeure DESC LIMIT 1")  # Récupérer la dernière entrée
    data = c.fetchone()
    conn.close()
...
# Appeler la fonction pour sauvegarder les données dans un fichier JSON
save_to_json()
